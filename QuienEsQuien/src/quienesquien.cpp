...
QuienEsQuien::QuienEsQuien(){
	// En principio, no hay que inicializar nada, ya que
	// la inicialización de los atributos de clase se hace
	// explícitamente llamando a métodos de la clase en el
	// main
}
...
void QuienEsQuien::limpiar(){
	// No hay que limpiar nada, no uso memoria dinámica.
	// La clase bintree usa memoria dinámica en los nodos
	// pero el destructor se llama automáticamente al destruirse
...
void QuienEsQuien::construccion_basica(bintree<Pregunta> & arbol, 
	const bintree<Pregunta>::node & nodo_actual,
	vector<bool> & camino, const int & num_atributo)
{

	// rama izquierda
	camino.push_back(true);
	set<string> coincidencias = personajes_coincidentes(camino);
	int num_coincidencias = coincidencias.size();
	Pregunta nuevo_nodo;

	if (num_coincidencias > 1) // Queda más de un personaje por averiguar
	{
		nuevo_nodo = Pregunta(atributos[num_atributo], num_coincidencias);
		arbol.insert_left(nodo_actual, nuevo_nodo);
		// Seguimos procesando por la izq
		construccion_basica(arbol, nodo_actual.left(), camino, num_atributo+1);
	}
	else if (num_coincidencias == 1) // Queda 1 personaje, es una hoja
	{
		nuevo_nodo = Pregunta(*coincidencias.begin(), 1);
		arbol.insert_left(nodo_actual, nuevo_nodo);
	}
	...
	// rama derecha
	camino.push_back(false);
	coincidencias = personajes_coincidentes(camino);
	num_coincidencias = coincidencias.size();

	if (num_coincidencias > 1) // Queda más de un personaje por averiguar
	{
		nuevo_nodo = Pregunta(atributos[num_atributo], num_coincidencias);
		arbol.insert_right(nodo_actual, nuevo_nodo);
		// Seguimos procesando por la der
		construccion_basica(arbol, nodo_actual.right(), camino, num_atributo+1);
	}
	else if (num_coincidencias == 1) // Queda 1 personaje, es una hoja
	{
		nuevo_nodo = Pregunta(*coincidencias.begin(), 1);
...
bintree<Pregunta> QuienEsQuien::crear_arbol()
{
	...
	Pregunta p_raiz(atributos[0], personajes.size());
	bintree<Pregunta> arbol(p_raiz);

	// Creamos el resto del árbol de manera recursiva
	vector<bool> camino;
	construccion_basica(arbol, arbol.root(), camino, 1);

	return arbol;
}

set<string> QuienEsQuien::personajes_coincidentes(const vector<bool> & respuestas)
{
	set<string> personajes_coincidentes;
	for (int personaje = 0; personaje < personajes.size(); personaje++){
		int atrib = 0;
		bool coincidencia = true;
		while (coincidencia && atrib < respuestas.size()){
			if (respuestas[atrib] != tablero[personaje][atrib])
				coincidencia = false;
			atrib++;
		}

		if (coincidencia)
			personajes_coincidentes.insert(personajes[personaje]);
	}

	return personajes_coincidentes;
}
...
void QuienEsQuien::iniciar_juego(){
	...
	while (!(*jugada_actual).es_personaje())
	{
		...
		cout << "Puede ser uno de estos personajes: ";
	...
	cout << endl << "Tu personaje es: " << (*jugada_actual).obtener_personaje() << endl;
}

set<string> QuienEsQuien::informacion_jugada(const bintree<Pregunta>::node jugada_actual){
	
	set<string> personajes_levantados;
	bintree<Pregunta> sub_arbol;
	bintree<Pregunta>::const_preorder_iterator it = arbol.begin_preorder();
	...
	for (it = sub_arbol.begin_preorder(); it != sub_arbol.end_preorder(); ++it)
		if ((*it).es_personaje())
...
void QuienEsQuien::eliminar_nodos_redundantes(){
	...
	while(!n.null())
	{
		// solo un hijo a la izquierda
		if (!n.left().null() && n.right().null())
		{
			arbol.prune_left(n,subarbol);
			arbol.replace_subtree(n, subarbol, subarbol.root());
		}
		// solo un hijo a la derecha
		else if (!n.right().null() && n.left().null())
		{
			arbol.prune_right(n,subarbol);
			arbol.replace_subtree(n, subarbol, subarbol.root());
		} else {
			// Seleccionamos el siguiente nodo en preorden
			if (!n.left().null())
				n = n.left();
				...
				while (!n.parent().null() &&
				    (n.parent().right() == n || n.parent().right().null()))
				    n = n.parent();
				if (n.parent().null())
					n = typename bintree<Pregunta>::node();
				else
					n = n.parent().right();
			}
		}
...
void QuienEsQuien::suma_profundidades(const bintree<Pregunta>::node n, 
	int & prof_parcial, int & sum_prof)
{
	prof_parcial += 1;

	if ((*n).es_personaje())
		sum_prof+=prof_parcial-1; // -1 para equilibrar el +1 que se hace en el nodo raíz
	else{
		if (!n.left().null()){ // Procesamos la izquierda
			suma_profundidades(n.left(),prof_parcial,sum_prof);
			prof_parcial-=1; // subimos al padre
		}
		if (!n.right().null()){ // Procesamos la derecha
			suma_profundidades(n.right(),prof_parcial,sum_prof);
			prof_parcial-=1; // subimos al padre
		}
	}
}
...
float QuienEsQuien::profundidad_promedio_hojas(){
	...
	int sum_prof = 0;
	int sum_par = 0;
	suma_profundidades(arbol.root(),sum_par,sum_prof);

	return ((float)sum_prof/personajes.size());
}		
